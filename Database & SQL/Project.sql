...
alter view VDoctor_retire
as
select dr.Fname+' '+dr.Lname as fullname,d.Dept_Name,dr.Age
from Doctor as dr, Departement as d
where dr.Dept_ID=d.Dept_ID and dr.Age>55
select * from VDoctor_retire


--Make a rule on age of dr is less than 80
...
--Number of rooms per departement
select Dept_Name,count(Room_No) as No_of_rooms
from Departement ,Room
where Departement.Dept_ID=Room.Dept_ID
group by Dept_Name
...
alter view nurse_retire
as
select Nurse_ID , Fname , Lname,Age
from Nurse 
WHERE Age>55
...
----
alter procedure Medicine_with_the_min_exp_date as
